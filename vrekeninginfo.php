...
//
// Table class for vrekening
//
class cvrekening extends cTable {
	var $group;
	var $id;
	var $rekening;
	var $tipe;
	...
	function __construct() {
		...
		$this->TableVar = 'vrekening';
		$this->TableName = 'vrekening';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vrekening`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// group
		$this->group = new cField('vrekening', 'vrekening', 'x_group', 'group', '`group`', '`group`', 20, -1, FALSE, '`group`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// id
		$this->id = new cField('vrekening', 'vrekening', 'x_id', 'id', '`id`', '`id`', 200, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id->Sortable = TRUE; // Allow sort
		$this->fields['id'] = &$this->id;

		// rekening
		$this->rekening = new cField('vrekening', 'vrekening', 'x_rekening', 'rekening', '`rekening`', '`rekening`', 200, -1, FALSE, '`rekening`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rekening->Sortable = TRUE; // Allow sort
		$this->fields['rekening'] = &$this->rekening;

		// tipe
		$this->tipe = new cField('vrekening', 'vrekening', 'x_tipe', 'tipe', '`tipe`', '`tipe`', 200, -1, FALSE, '`tipe`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->tipe->Sortable = TRUE; // Allow sort
		$this->fields['tipe'] = &$this->tipe;

		// posisi
		$this->posisi = new cField('vrekening', 'vrekening', 'x_posisi', 'posisi', '`posisi`', '`posisi`', 200, -1, FALSE, '`posisi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->posisi->Sortable = TRUE; // Allow sort
		$this->fields['posisi'] = &$this->posisi;

		// laporan
		$this->laporan = new cField('vrekening', 'vrekening', 'x_laporan', 'laporan', '`laporan`', '`laporan`', 200, -1, FALSE, '`laporan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->laporan->Sortable = TRUE; // Allow sort
		$this->fields['laporan'] = &$this->laporan;

		// status
		$this->status = new cField('vrekening', 'vrekening', 'x_status', 'status', '`status`', '`status`', 200, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->status->Sortable = TRUE; // Allow sort
		$this->fields['status'] = &$this->status;

		// parent
		$this->parent = new cField('vrekening', 'vrekening', 'x_parent', 'parent', '`parent`', '`parent`', 200, -1, FALSE, '`parent`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->parent->Sortable = TRUE; // Allow sort
		$this->fields['parent'] = &$this->parent;

		// keterangan
		$this->keterangan = new cField('vrekening', 'vrekening', 'x_keterangan', 'keterangan', '`keterangan`', '`keterangan`', 200, -1, FALSE, '`keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->keterangan->Sortable = TRUE; // Allow sort
		$this->fields['keterangan'] = &$this->keterangan;

		// active
		$this->active = new cField('vrekening', 'vrekening', 'x_active', 'active', '`active`', '`active`', 202, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "trekening") {
			if ($this->parent->getSessionValue() <> "")
				$sMasterFilter .= "`id`=" . ew_QuotedValue($this->parent->getSessionValue(), EW_DATATYPE_STRING, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "trekening") {
			if ($this->parent->getSessionValue() <> "")
				$sDetailFilter .= "`parent`=" . ew_QuotedValue($this->parent->getSessionValue(), EW_DATATYPE_STRING, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_trekening() {
		return "`id`='@id@'";
	}

	// Detail filter
	function SqlDetailFilter_trekening() {
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`vrekening`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("vrekeningview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("vrekeningview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "vrekeningadd.php?" . $this->UrlParm($parm);
		else
			$url = "vrekeningadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("vrekeningedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("vrekeningadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("vrekeningdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "trekening" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_id=" . urlencode($this->parent->CurrentValue);
		}
		return $url;
	}
