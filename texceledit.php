...
$texcel_edit = NULL; // Initialize page object first

class ctexcel_edit extends ctexcel {
	...
	function Page_Main() {
		...
		if ($this->SHEET->CurrentValue == "") {
			$this->Page_Terminate("texcellist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// B
			$this->B->SetDbValueDef($rsnew, $this->B->CurrentValue, "", $this->B->ReadOnly);

			// C
			$this->C->SetDbValueDef($rsnew, $this->C->CurrentValue, "", $this->C->ReadOnly);

			// D
			$this->D->SetDbValueDef($rsnew, $this->D->CurrentValue, "", $this->D->ReadOnly);

			// E
			$this->E->SetDbValueDef($rsnew, $this->E->CurrentValue, "", $this->E->ReadOnly);

			// F
			$this->F->SetDbValueDef($rsnew, $this->F->CurrentValue, "", $this->F->ReadOnly);

			// G
			$this->G->SetDbValueDef($rsnew, $this->G->CurrentValue, "", $this->G->ReadOnly);

			// H
			$this->H->SetDbValueDef($rsnew, $this->H->CurrentValue, "", $this->H->ReadOnly);

			// I
			$this->_I->SetDbValueDef($rsnew, $this->_I->CurrentValue, "", $this->_I->ReadOnly);

			// J
			$this->J->SetDbValueDef($rsnew, $this->J->CurrentValue, "", $this->J->ReadOnly);
			...
			// O
			$this->O->SetDbValueDef($rsnew, $this->O->CurrentValue, "", $this->O->ReadOnly);

			// P
			$this->P->SetDbValueDef($rsnew, $this->P->CurrentValue, "", $this->P->ReadOnly);

			// Q
			$this->Q->SetDbValueDef($rsnew, $this->Q->CurrentValue, "", $this->Q->ReadOnly);

			// R
			$this->R->SetDbValueDef($rsnew, $this->R->CurrentValue, "", $this->R->ReadOnly);

			// S
			$this->S->SetDbValueDef($rsnew, $this->S->CurrentValue, "", $this->S->ReadOnly);

			// T
			$this->T->SetDbValueDef($rsnew, $this->T->CurrentValue, "", $this->T->ReadOnly);

			// U
			$this->U->SetDbValueDef($rsnew, $this->U->CurrentValue, "", $this->U->ReadOnly);
			...
			// X
			$this->X->SetDbValueDef($rsnew, $this->X->CurrentValue, "", $this->X->ReadOnly);

			// Y
			$this->Y->SetDbValueDef($rsnew, $this->Y->CurrentValue, "", $this->Y->ReadOnly);

			// Z
			$this->Z->SetDbValueDef($rsnew, $this->Z->CurrentValue, "", $this->Z->ReadOnly);
...
// Create page object
if (!isset($texcel_edit)) $texcel_edit = new ctexcel_edit();

// Page init
$texcel_edit->Page_Init();

// Page main
$texcel_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$texcel_edit->Page_Render();
?>
...
var CurrentForm = ftexceledit = new ew_Form("ftexceledit", "edit");

// Validate form
ftexceledit.Validate = function() {
...
ftexceledit.Form_CustomValidate = 
...
ftexceledit.ValidateRequired = true;
<?php } else { ?>
ftexceledit.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $texcel_edit->ShowPageHeader(); ?>
<?php
$texcel_edit->ShowMessage();
?>
<form name="ftexceledit" id="ftexceledit" class="<?php echo $texcel_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($texcel_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $texcel_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $texcel->SHEET->EditValue ?></p></span>
</span>
<input type="hidden" data-table="texcel" data-field="x_SHEET" name="x_SHEET" id="x_SHEET" value="<?php echo ew_HtmlEncode($texcel->SHEET->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $texcel->NOMOR->EditValue ?></p></span>
</span>
<input type="hidden" data-table="texcel" data-field="x_NOMOR" name="x_NOMOR" id="x_NOMOR" value="<?php echo ew_HtmlEncode($texcel->NOMOR->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $texcel_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
