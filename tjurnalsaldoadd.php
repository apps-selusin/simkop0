...
$tjurnalsaldo_add = NULL; // Initialize page object first

class ctjurnalsaldo_add extends ctjurnalsaldo {
	...
	// Page object name
	var $PageObjName = 'tjurnalsaldo_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tjurnalsaldoview.php")
...
// Create page object
if (!isset($tjurnalsaldo_add)) $tjurnalsaldo_add = new ctjurnalsaldo_add();

// Page init
$tjurnalsaldo_add->Page_Init();

// Page main
$tjurnalsaldo_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tjurnalsaldo_add->Page_Render();
?>
...
var CurrentForm = ftjurnalsaldoadd = new ew_Form("ftjurnalsaldoadd", "add");

// Validate form
ftjurnalsaldoadd.Validate = function() {
...
ftjurnalsaldoadd.Form_CustomValidate = 
...
ftjurnalsaldoadd.ValidateRequired = true;
<?php } else { ?>
ftjurnalsaldoadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftjurnalsaldoadd.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftjurnalsaldoadd.Lists["x_active"].Options = <?php echo json_encode($tjurnalsaldo->active->Options()) ?>;
...
<?php if (!$tjurnalsaldo_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $tjurnalsaldo_add->ShowPageHeader(); ?>
<?php
$tjurnalsaldo_add->ShowMessage();
?>
<form name="ftjurnalsaldoadd" id="ftjurnalsaldoadd" class="<?php echo $tjurnalsaldo_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tjurnalsaldo_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tjurnalsaldo_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="tjurnalsaldo" data-field="x_id" name="x_id" id="x_id" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalsaldo->id->getPlaceHolder()) ?>" value="<?php echo $tjurnalsaldo->id->EditValue ?>"<?php echo $tjurnalsaldo->id->EditAttributes() ?>>
</span>
...
<input type="text" data-table="tjurnalsaldo" data-field="x_nomor" name="x_nomor" id="x_nomor" size="30" placeholder="<?php echo ew_HtmlEncode($tjurnalsaldo->nomor->getPlaceHolder()) ?>" value="<?php echo $tjurnalsaldo->nomor->EditValue ?>"<?php echo $tjurnalsaldo->nomor->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tjurnalsaldo_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ftjurnalsaldoadd.Init();
</script>
<?php
$tjurnalsaldo_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tjurnalsaldo_add->Page_Terminate();
?>
