...
$tpinjaman_edit = NULL; // Initialize page object first

class ctpinjaman_edit extends ctpinjaman {
	...
	// Page object name
	var $PageObjName = 'tpinjaman_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("tpinjamanlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// agunan
			$this->agunan->SetDbValueDef($rsnew, $this->agunan->CurrentValue, "", $this->agunan->ReadOnly);

			// dataagunan1
			$this->dataagunan1->SetDbValueDef($rsnew, $this->dataagunan1->CurrentValue, "", $this->dataagunan1->ReadOnly);

			// dataagunan2
			$this->dataagunan2->SetDbValueDef($rsnew, $this->dataagunan2->CurrentValue, "", $this->dataagunan2->ReadOnly);

			// dataagunan3
			$this->dataagunan3->SetDbValueDef($rsnew, $this->dataagunan3->CurrentValue, "", $this->dataagunan3->ReadOnly);

			// dataagunan4
			$this->dataagunan4->SetDbValueDef($rsnew, $this->dataagunan4->CurrentValue, "", $this->dataagunan4->ReadOnly);

			// dataagunan5
			$this->dataagunan5->SetDbValueDef($rsnew, $this->dataagunan5->CurrentValue, "", $this->dataagunan5->ReadOnly);

			// saldobekusimpanan
			$this->saldobekusimpanan->SetDbValueDef($rsnew, $this->saldobekusimpanan->CurrentValue, 0, $this->saldobekusimpanan->ReadOnly);

			// saldobekuminimal
			$this->saldobekuminimal->SetDbValueDef($rsnew, $this->saldobekuminimal->CurrentValue, 0, $this->saldobekuminimal->ReadOnly);
			...
			// bungapersen
			$this->bungapersen->SetDbValueDef($rsnew, $this->bungapersen->CurrentValue, 0, $this->bungapersen->ReadOnly);

			// administrasi
			$this->administrasi->SetDbValueDef($rsnew, $this->administrasi->CurrentValue, 0, $this->administrasi->ReadOnly);

			// administrasipersen
			$this->administrasipersen->SetDbValueDef($rsnew, $this->administrasipersen->CurrentValue, 0, $this->administrasipersen->ReadOnly);

			// asuransi
			$this->asuransi->SetDbValueDef($rsnew, $this->asuransi->CurrentValue, 0, $this->asuransi->ReadOnly);

			// notaris
			$this->notaris->SetDbValueDef($rsnew, $this->notaris->CurrentValue, 0, $this->notaris->ReadOnly);

			// biayamaterai
			$this->biayamaterai->SetDbValueDef($rsnew, $this->biayamaterai->CurrentValue, 0, $this->biayamaterai->ReadOnly);

			// potongansaldobeku
			$this->potongansaldobeku->SetDbValueDef($rsnew, $this->potongansaldobeku->CurrentValue, 0, $this->potongansaldobeku->ReadOnly);
			...
			// totalterima
			$this->totalterima->SetDbValueDef($rsnew, $this->totalterima->CurrentValue, 0, $this->totalterima->ReadOnly);

			// totalterimaauto
			$this->totalterimaauto->SetDbValueDef($rsnew, $this->totalterimaauto->CurrentValue, 0, $this->totalterimaauto->ReadOnly);
...
// Create page object
if (!isset($tpinjaman_edit)) $tpinjaman_edit = new ctpinjaman_edit();

// Page init
$tpinjaman_edit->Page_Init();

// Page main
$tpinjaman_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tpinjaman_edit->Page_Render();
?>
...
var CurrentForm = ftpinjamanedit = new ew_Form("ftpinjamanedit", "edit");

// Validate form
ftpinjamanedit.Validate = function() {
...
ftpinjamanedit.Form_CustomValidate = 
...
ftpinjamanedit.ValidateRequired = true;
<?php } else { ?>
ftpinjamanedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftpinjamanedit.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftpinjamanedit.Lists["x_active"].Options = <?php echo json_encode($tpinjaman->active->Options()) ?>;
...
<?php } ?>
<?php $tpinjaman_edit->ShowPageHeader(); ?>
<?php
$tpinjaman_edit->ShowMessage();
?>
<form name="ftpinjamanedit" id="ftpinjamanedit" class="<?php echo $tpinjaman_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tpinjaman_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tpinjaman_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $tpinjaman->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="tpinjaman" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($tpinjaman->id->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tpinjaman_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
