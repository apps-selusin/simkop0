...
$trekening3_delete = NULL; // Initialize page object first

class ctrekening3_delete extends ctrekening3 {
	...
	// Page object name
	var $PageObjName = 'trekening3_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("trekening3list.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in trekening3 class, trekening3info.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("trekening3list.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($trekening3_delete)) $trekening3_delete = new ctrekening3_delete();

// Page init
$trekening3_delete->Page_Init();

// Page main
$trekening3_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$trekening3_delete->Page_Render();
?>
...
var CurrentForm = ftrekening3delete = new ew_Form("ftrekening3delete", "delete");

// Form_CustomValidate event
ftrekening3delete.Form_CustomValidate = 
...
ftrekening3delete.ValidateRequired = true;
<?php } else { ?>
ftrekening3delete.ValidateRequired = false; 
<?php } ?>
...
<?php $trekening3_delete->ShowPageHeader(); ?>
<?php
$trekening3_delete->ShowMessage();
?>
<form name="ftrekening3delete" id="ftrekening3delete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($trekening3_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $trekening3_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="trekening3">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($trekening3_delete->RecKeys as $key) { ?>
...
<?php if ($trekening3->group->Visible) { // group ?>
		<th><span id="elh_trekening3_group" class="trekening3_group"><?php echo $trekening3->group->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening3->id1->Visible) { // id1 ?>
		<th><span id="elh_trekening3_id1" class="trekening3_id1"><?php echo $trekening3->id1->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening3->id2->Visible) { // id2 ?>
		<th><span id="elh_trekening3_id2" class="trekening3_id2"><?php echo $trekening3->id2->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening3->rekening->Visible) { // rekening ?>
		<th><span id="elh_trekening3_rekening" class="trekening3_rekening"><?php echo $trekening3->rekening->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening3->keterangan->Visible) { // keterangan ?>
		<th><span id="elh_trekening3_keterangan" class="trekening3_keterangan"><?php echo $trekening3->keterangan->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening3->status->Visible) { // status ?>
		<th><span id="elh_trekening3_status" class="trekening3_status"><?php echo $trekening3->status->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$trekening3_delete->RecCnt = 0;
$i = 0;
while (!$trekening3_delete->Recordset->EOF) {
	$trekening3_delete->RecCnt++;
	$trekening3_delete->RowCnt++;

	// Set row properties
	$trekening3->ResetAttrs();
	$trekening3->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$trekening3_delete->LoadRowValues($trekening3_delete->Recordset);

	// Render row
	$trekening3_delete->RenderRow();
?>
	<tr<?php echo $trekening3->RowAttributes() ?>>
<?php if ($trekening3->group->Visible) { // group ?>
		<td<?php echo $trekening3->group->CellAttributes() ?>>
<span id="el<?php echo $trekening3_delete->RowCnt ?>_trekening3_group" class="trekening3_group">
...
<?php } ?>
<?php if ($trekening3->id1->Visible) { // id1 ?>
		<td<?php echo $trekening3->id1->CellAttributes() ?>>
<span id="el<?php echo $trekening3_delete->RowCnt ?>_trekening3_id1" class="trekening3_id1">
...
<?php } ?>
<?php if ($trekening3->id2->Visible) { // id2 ?>
		<td<?php echo $trekening3->id2->CellAttributes() ?>>
<span id="el<?php echo $trekening3_delete->RowCnt ?>_trekening3_id2" class="trekening3_id2">
...
<?php } ?>
<?php if ($trekening3->rekening->Visible) { // rekening ?>
		<td<?php echo $trekening3->rekening->CellAttributes() ?>>
<span id="el<?php echo $trekening3_delete->RowCnt ?>_trekening3_rekening" class="trekening3_rekening">
...
<?php } ?>
<?php if ($trekening3->keterangan->Visible) { // keterangan ?>
		<td<?php echo $trekening3->keterangan->CellAttributes() ?>>
<span id="el<?php echo $trekening3_delete->RowCnt ?>_trekening3_keterangan" class="trekening3_keterangan">
...
<?php } ?>
<?php if ($trekening3->status->Visible) { // status ?>
		<td<?php echo $trekening3->status->CellAttributes() ?>>
<span id="el<?php echo $trekening3_delete->RowCnt ?>_trekening3_status" class="trekening3_status">
...
<?php
	$trekening3_delete->Recordset->MoveNext();
}
$trekening3_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $trekening3_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftrekening3delete.Init();
</script>
<?php
$trekening3_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$trekening3_delete->Page_Terminate();
?>
