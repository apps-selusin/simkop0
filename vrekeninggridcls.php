...
$vrekening_grid = NULL; // Initialize page object first

class cvrekening_grid extends cvrekening {
	...
	// Page object name
	var $PageObjName = 'vrekening_grid';

	// Grid form hidden field names
	var $FormName = 'fvrekeninggrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["vrekening"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_group") && $objForm->HasValue("o_group") && $this->group->CurrentValue <> $this->group->OldValue)
			return FALSE;
		...
		if ($objForm->HasValue("x_rekening") && $objForm->HasValue("o_rekening") && $this->rekening->CurrentValue <> $this->rekening->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_tipe") && $objForm->HasValue("o_tipe") && $this->tipe->CurrentValue <> $this->tipe->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_posisi") && $objForm->HasValue("o_posisi") && $this->posisi->CurrentValue <> $this->posisi->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_laporan") && $objForm->HasValue("o_laporan") && $this->laporan->CurrentValue <> $this->laporan->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		$this->group->setOldValue($objForm->GetValue("o_group"));
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "trekening") {
			...
			if ($GLOBALS["trekening"]->EventCancelled) $this->EventCancelled = TRUE;
		}
