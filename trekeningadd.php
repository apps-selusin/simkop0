...
$trekening_add = NULL; // Initialize page object first

class ctrekening_add extends ctrekening {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "trekeningview.php")
	...
	function ValidateForm() {
		...
		if (in_array("vrekening", $DetailTblVar) && $GLOBALS["vrekening"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("vrekening", $DetailTblVar) && $GLOBALS["vrekening"]->DetailAdd) {
				$GLOBALS["vrekening"]->parent->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["vrekening_grid"])) $GLOBALS["vrekening_grid"] = new cvrekening_grid(); // Get detail page object
				$AddRow = $GLOBALS["vrekening_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["vrekening"]->parent->setSessionValue(""); // Clear master key if insert failed
			}
		}
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("vrekening", $DetailTblVar)) {
				...
				if ($GLOBALS["vrekening_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["vrekening_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["vrekening_grid"]->CurrentMode = "add";
					$GLOBALS["vrekening_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($trekening_add)) $trekening_add = new ctrekening_add();

// Page init
$trekening_add->Page_Init();

// Page main
$trekening_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$trekening_add->Page_Render();
?>
...
var CurrentForm = ftrekeningadd = new ew_Form("ftrekeningadd", "add");

// Validate form
ftrekeningadd.Validate = function() {
...
ftrekeningadd.Form_CustomValidate = 
...
ftrekeningadd.ValidateRequired = true;
<?php } else { ?>
ftrekeningadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftrekeningadd.Lists["x_group"] = {"LinkField":"x_group","Ajax":true,"AutoFill":false,"DisplayFields":["x_rekening","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"trekening"};
ftrekeningadd.Lists["x_tipe[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftrekeningadd.Lists["x_tipe[]"].Options = <?php echo json_encode($trekening->tipe->Options()) ?>;
ftrekeningadd.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftrekeningadd.Lists["x_active"].Options = <?php echo json_encode($trekening->active->Options()) ?>;
...
<?php } ?>
<?php $trekening_add->ShowPageHeader(); ?>
<?php
$trekening_add->ShowMessage();
?>
<form name="ftrekeningadd" id="ftrekeningadd" class="<?php echo $trekening_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($trekening_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $trekening_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="trekening" data-field="x_id" name="x_id" id="x_id" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($trekening->id->getPlaceHolder()) ?>" value="<?php echo $trekening->id->EditValue ?>"<?php echo $trekening->id->EditAttributes() ?>>
</span>
...
<?php
	if (in_array("vrekening", explode(",", $trekening->getCurrentDetailTable())) && $vrekening->DetailAdd) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $trekening_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
