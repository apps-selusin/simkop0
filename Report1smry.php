...
//		$item->Visible = FALSE;
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			// Load default values
			$this->SetSessionDropDownValue($this->group->DropDownValue, $this->group->SearchOperator, 'group'); // Field group
			...
			// Field group
			if ($this->GetDropDownValue($this->group)) {
				$bSetupFilter = TRUE;
			} elseif ($this->group->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_Report1_group'])) {
				$bSetupFilter = TRUE;
			}
		...
		// Restore session
		if ($bRestoreSession) {
			$this->GetSessionDropDownValue($this->group); // Field group
		}
		...
		// Build SQL
		$this->BuildDropDownFilter($this->group, $sFilter, $this->group->SearchOperator, FALSE, TRUE); // Field group
		...
		// Field group
		ewr_LoadDropDownList($this->group->DropDownList, $this->group->DropDownValue);
		return $sFilter;
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field group
		$this->group->DefaultDropDownValue = EWR_INIT_VALUE;
		if (!$this->SearchCommand) $this->group->DropDownValue = $this->group->DefaultDropDownValue;
		/**
	...
	function CheckFilter() {
		...
		// Check group extended filter
		if ($this->NonTextFilterApplied($this->group))
			return TRUE;
		return FALSE;
	}
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildDropDownFilter($this->group, $sExtWrk, $this->group->SearchOperator);
		$sFilter = "";
		if ($sExtWrk <> "")
		...
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->group->FldCaption() . "</span>" . $sFilter . "</div>";
		$divstyle = "";
		$divdataclass = "";
	...
	function GetFilterList() {
		...
		// Field group
		$sWrk = "";
		$sWrk = ($this->group->DropDownValue <> EWR_INIT_VALUE) ? $this->group->DropDownValue : "";
		if (is_array($sWrk))
			$sWrk = implode("||", $sWrk);
		if ($sWrk <> "")
			$sWrk = "\"sv_group\":\"" . ewr_JsEncode2($sWrk) . "\"";
		if ($sWrk <> "") {
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sv_group", $filter)) {
			$sWrk = $filter["sv_group"];
			if (strpos($sWrk, "||") !== FALSE)
				$sWrk = explode("||", $sWrk);
			$this->SetSessionDropDownValue($sWrk, @$filter["so_group"], "group");
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "group");
		}
		return TRUE;
	}
...
// Use Ajax
fReport1summary.Lists["sv_group"] = {"LinkField":"sv_group","Ajax":true,"DisplayFields":["sv_group","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if (!$Page->DrillDown) { ?>
<script type="text/javascript">
...
<div id="c_group" class="ewCell form-group">
	<label for="sv_group" class="ewSearchCaption ewLabel"><?php echo $Page->group->FldCaption() ?></label>
	<span class="ewSearchField">
<?php $Page->group->EditAttrs["onchange"] = "ewrForms(this).Submit(); " . @$Page->group->EditAttrs["onchange"]; ?>
<?php ewr_PrependClass($Page->group->EditAttrs["class"], "form-control"); ?>
<select data-table="Report1" data-field="x_group" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->group->DisplayValueSeparator) ? json_encode($Page->group->DisplayValueSeparator) : $Page->group->DisplayValueSeparator) ?>" id="sv_group" name="sv_group"<?php echo $Page->group->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
	$cntf = is_array($Page->group->AdvancedFilters) ? count($Page->group->AdvancedFilters) : 0;
	$cntd = is_array($Page->group->DropDownList) ? count($Page->group->DropDownList) : 0;
	$totcnt = $cntf + $cntd;
	$wrkcnt = 0;
	if ($cntf > 0) {
		foreach ($Page->group->AdvancedFilters as $filter) {
			if ($filter->Enabled) {
				$selwrk = ewr_MatchedFilterValue($Page->group->DropDownValue, $filter->ID) ? " selected" : "";
?>
...
<?php
	...
	for ($i = 0; $i < $cntd; $i++) {
		$selwrk = " selected";
?>
<option value="<?php echo $Page->group->DropDownList[$i] ?>"<?php echo $selwrk ?>><?php echo ewr_DropDownDisplayValue($Page->group->DropDownList[$i], "", 0) ?></option>
<?php
		$wrkcnt += 1;
	}
?>
</select>
<input type="hidden" name="s_sv_group" id="s_sv_group" value="<?php echo $Page->group->LookupFilterQuery() ?>"></span>
</div>
</div>
</div>
</form>
