...
$tjurnalkasbank_add = NULL; // Initialize page object first

class ctjurnalkasbank_add extends ctjurnalkasbank {
	...
	// Page object name
	var $PageObjName = 'tjurnalkasbank_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("tjurnalkasbanklist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "tjurnalkasbanklist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to list page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "tjurnalkasbankview.php")
...
// Create page object
if (!isset($tjurnalkasbank_add)) $tjurnalkasbank_add = new ctjurnalkasbank_add();

// Page init
$tjurnalkasbank_add->Page_Init();

// Page main
$tjurnalkasbank_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tjurnalkasbank_add->Page_Render();
?>
...
var CurrentForm = ftjurnalkasbankadd = new ew_Form("ftjurnalkasbankadd", "add");

// Validate form
ftjurnalkasbankadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->tanggal->FldCaption(), $tjurnalkasbank->tanggal->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_tanggal");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($tjurnalkasbank->tanggal->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_periode");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->periode->FldCaption(), $tjurnalkasbank->periode->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_id");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->id->FldCaption(), $tjurnalkasbank->id->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_nomor");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->nomor->FldCaption(), $tjurnalkasbank->nomor->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_nomor");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($tjurnalkasbank->nomor->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_transaksi");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->transaksi->FldCaption(), $tjurnalkasbank->transaksi->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_referensi");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->referensi->FldCaption(), $tjurnalkasbank->referensi->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_model");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->model->FldCaption(), $tjurnalkasbank->model->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_rekening");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->rekening->FldCaption(), $tjurnalkasbank->rekening->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_debet");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->debet->FldCaption(), $tjurnalkasbank->debet->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_debet");
			if (elm && !ew_CheckNumber(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($tjurnalkasbank->debet->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_credit");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->credit->FldCaption(), $tjurnalkasbank->credit->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_credit");
			if (elm && !ew_CheckNumber(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($tjurnalkasbank->credit->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_kantor");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->kantor->FldCaption(), $tjurnalkasbank->kantor->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_keterangan");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->keterangan->FldCaption(), $tjurnalkasbank->keterangan->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_active");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->active->FldCaption(), $tjurnalkasbank->active->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_ip");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->ip->FldCaption(), $tjurnalkasbank->ip->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_status");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->status->FldCaption(), $tjurnalkasbank->status->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_user");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->user->FldCaption(), $tjurnalkasbank->user->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_created");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->created->FldCaption(), $tjurnalkasbank->created->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_created");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($tjurnalkasbank->created->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_modified");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $tjurnalkasbank->modified->FldCaption(), $tjurnalkasbank->modified->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_modified");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($tjurnalkasbank->modified->FldErrMsg()) ?>");
...
ftjurnalkasbankadd.Form_CustomValidate = 
...
ftjurnalkasbankadd.ValidateRequired = true;
<?php } else { ?>
ftjurnalkasbankadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftjurnalkasbankadd.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftjurnalkasbankadd.Lists["x_active"].Options = <?php echo json_encode($tjurnalkasbank->active->Options()) ?>;
...
<?php if (!$tjurnalkasbank_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $tjurnalkasbank_add->ShowPageHeader(); ?>
<?php
$tjurnalkasbank_add->ShowMessage();
?>
<form name="ftjurnalkasbankadd" id="ftjurnalkasbankadd" class="<?php echo $tjurnalkasbank_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tjurnalkasbank_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tjurnalkasbank_add->Token ?>">
<?php } ?>
...
<?php if ($tjurnalkasbank->tanggal->Visible) { // tanggal ?>
	<div id="r_tanggal" class="form-group">
		<label id="elh_tjurnalkasbank_tanggal" for="x_tanggal" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->tanggal->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->tanggal->CellAttributes() ?>>
<span id="el_tjurnalkasbank_tanggal">
<input type="text" data-table="tjurnalkasbank" data-field="x_tanggal" name="x_tanggal" id="x_tanggal" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->tanggal->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->tanggal->EditValue ?>"<?php echo $tjurnalkasbank->tanggal->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->tanggal->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->periode->Visible) { // periode ?>
	<div id="r_periode" class="form-group">
		<label id="elh_tjurnalkasbank_periode" for="x_periode" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->periode->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->periode->CellAttributes() ?>>
<span id="el_tjurnalkasbank_periode">
<input type="text" data-table="tjurnalkasbank" data-field="x_periode" name="x_periode" id="x_periode" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->periode->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->periode->EditValue ?>"<?php echo $tjurnalkasbank->periode->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->periode->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_tjurnalkasbank_id" for="x_id" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->id->CellAttributes() ?>>
<span id="el_tjurnalkasbank_id">
<input type="text" data-table="tjurnalkasbank" data-field="x_id" name="x_id" id="x_id" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->id->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->id->EditValue ?>"<?php echo $tjurnalkasbank->id->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->nomor->Visible) { // nomor ?>
	<div id="r_nomor" class="form-group">
		<label id="elh_tjurnalkasbank_nomor" for="x_nomor" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->nomor->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->nomor->CellAttributes() ?>>
<span id="el_tjurnalkasbank_nomor">
<input type="text" data-table="tjurnalkasbank" data-field="x_nomor" name="x_nomor" id="x_nomor" size="30" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->nomor->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->nomor->EditValue ?>"<?php echo $tjurnalkasbank->nomor->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->nomor->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->transaksi->Visible) { // transaksi ?>
	<div id="r_transaksi" class="form-group">
		<label id="elh_tjurnalkasbank_transaksi" for="x_transaksi" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->transaksi->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->transaksi->CellAttributes() ?>>
<span id="el_tjurnalkasbank_transaksi">
<input type="text" data-table="tjurnalkasbank" data-field="x_transaksi" name="x_transaksi" id="x_transaksi" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->transaksi->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->transaksi->EditValue ?>"<?php echo $tjurnalkasbank->transaksi->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->transaksi->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->referensi->Visible) { // referensi ?>
	<div id="r_referensi" class="form-group">
		<label id="elh_tjurnalkasbank_referensi" for="x_referensi" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->referensi->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->referensi->CellAttributes() ?>>
<span id="el_tjurnalkasbank_referensi">
<input type="text" data-table="tjurnalkasbank" data-field="x_referensi" name="x_referensi" id="x_referensi" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->referensi->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->referensi->EditValue ?>"<?php echo $tjurnalkasbank->referensi->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->referensi->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->model->Visible) { // model ?>
	<div id="r_model" class="form-group">
		<label id="elh_tjurnalkasbank_model" for="x_model" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->model->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->model->CellAttributes() ?>>
<span id="el_tjurnalkasbank_model">
<input type="text" data-table="tjurnalkasbank" data-field="x_model" name="x_model" id="x_model" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->model->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->model->EditValue ?>"<?php echo $tjurnalkasbank->model->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->model->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->rekening->Visible) { // rekening ?>
	<div id="r_rekening" class="form-group">
		<label id="elh_tjurnalkasbank_rekening" for="x_rekening" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->rekening->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->rekening->CellAttributes() ?>>
<span id="el_tjurnalkasbank_rekening">
<input type="text" data-table="tjurnalkasbank" data-field="x_rekening" name="x_rekening" id="x_rekening" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->rekening->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->rekening->EditValue ?>"<?php echo $tjurnalkasbank->rekening->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->rekening->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->debet->Visible) { // debet ?>
	<div id="r_debet" class="form-group">
		<label id="elh_tjurnalkasbank_debet" for="x_debet" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->debet->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->debet->CellAttributes() ?>>
<span id="el_tjurnalkasbank_debet">
<input type="text" data-table="tjurnalkasbank" data-field="x_debet" name="x_debet" id="x_debet" size="30" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->debet->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->debet->EditValue ?>"<?php echo $tjurnalkasbank->debet->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->debet->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->credit->Visible) { // credit ?>
	<div id="r_credit" class="form-group">
		<label id="elh_tjurnalkasbank_credit" for="x_credit" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->credit->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->credit->CellAttributes() ?>>
<span id="el_tjurnalkasbank_credit">
<input type="text" data-table="tjurnalkasbank" data-field="x_credit" name="x_credit" id="x_credit" size="30" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->credit->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->credit->EditValue ?>"<?php echo $tjurnalkasbank->credit->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->credit->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->kantor->Visible) { // kantor ?>
	<div id="r_kantor" class="form-group">
		<label id="elh_tjurnalkasbank_kantor" for="x_kantor" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->kantor->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->kantor->CellAttributes() ?>>
<span id="el_tjurnalkasbank_kantor">
<input type="text" data-table="tjurnalkasbank" data-field="x_kantor" name="x_kantor" id="x_kantor" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->kantor->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->kantor->EditValue ?>"<?php echo $tjurnalkasbank->kantor->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->kantor->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->keterangan->Visible) { // keterangan ?>
	<div id="r_keterangan" class="form-group">
		<label id="elh_tjurnalkasbank_keterangan" for="x_keterangan" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->keterangan->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->keterangan->CellAttributes() ?>>
<span id="el_tjurnalkasbank_keterangan">
<input type="text" data-table="tjurnalkasbank" data-field="x_keterangan" name="x_keterangan" id="x_keterangan" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->keterangan->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->keterangan->EditValue ?>"<?php echo $tjurnalkasbank->keterangan->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->keterangan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->active->Visible) { // active ?>
	<div id="r_active" class="form-group">
		<label id="elh_tjurnalkasbank_active" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->active->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->active->CellAttributes() ?>>
<span id="el_tjurnalkasbank_active">
...
<?php echo $tjurnalkasbank->active->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->ip->Visible) { // ip ?>
	<div id="r_ip" class="form-group">
		<label id="elh_tjurnalkasbank_ip" for="x_ip" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->ip->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->ip->CellAttributes() ?>>
<span id="el_tjurnalkasbank_ip">
<input type="text" data-table="tjurnalkasbank" data-field="x_ip" name="x_ip" id="x_ip" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->ip->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->ip->EditValue ?>"<?php echo $tjurnalkasbank->ip->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->ip->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->status->Visible) { // status ?>
	<div id="r_status" class="form-group">
		<label id="elh_tjurnalkasbank_status" for="x_status" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->status->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->status->CellAttributes() ?>>
<span id="el_tjurnalkasbank_status">
<input type="text" data-table="tjurnalkasbank" data-field="x_status" name="x_status" id="x_status" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->status->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->status->EditValue ?>"<?php echo $tjurnalkasbank->status->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->status->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->user->Visible) { // user ?>
	<div id="r_user" class="form-group">
		<label id="elh_tjurnalkasbank_user" for="x_user" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->user->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->user->CellAttributes() ?>>
<span id="el_tjurnalkasbank_user">
<input type="text" data-table="tjurnalkasbank" data-field="x_user" name="x_user" id="x_user" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->user->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->user->EditValue ?>"<?php echo $tjurnalkasbank->user->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->user->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->created->Visible) { // created ?>
	<div id="r_created" class="form-group">
		<label id="elh_tjurnalkasbank_created" for="x_created" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->created->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->created->CellAttributes() ?>>
<span id="el_tjurnalkasbank_created">
<input type="text" data-table="tjurnalkasbank" data-field="x_created" name="x_created" id="x_created" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->created->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->created->EditValue ?>"<?php echo $tjurnalkasbank->created->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->created->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($tjurnalkasbank->modified->Visible) { // modified ?>
	<div id="r_modified" class="form-group">
		<label id="elh_tjurnalkasbank_modified" for="x_modified" class="col-sm-2 control-label ewLabel"><?php echo $tjurnalkasbank->modified->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $tjurnalkasbank->modified->CellAttributes() ?>>
<span id="el_tjurnalkasbank_modified">
<input type="text" data-table="tjurnalkasbank" data-field="x_modified" name="x_modified" id="x_modified" placeholder="<?php echo ew_HtmlEncode($tjurnalkasbank->modified->getPlaceHolder()) ?>" value="<?php echo $tjurnalkasbank->modified->EditValue ?>"<?php echo $tjurnalkasbank->modified->EditAttributes() ?>>
</span>
<?php echo $tjurnalkasbank->modified->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$tjurnalkasbank_add->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tjurnalkasbank_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ftjurnalkasbankadd.Init();
</script>
<?php
$tjurnalkasbank_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tjurnalkasbank_add->Page_Terminate();
?>
