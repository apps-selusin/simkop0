...
class crReport1 extends crTableBase {
	...
	function SetupLookupFilters($fld) {
		...
		case "x_group":
			$sSqlWrk = "";
		$sSqlWrk = "SELECT DISTINCT `group`, `group` AS `DispFld`, '' AS `DispFld2`, '' AS `DispFld3`, '' AS `DispFld4` FROM `vrekening2`";
		$sWhereWrk = "";
		$this->group->LookupFilters = array();
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "DB", "f0" => '`group` = {filter_value}', "t0" => "20", "fn0" => "", "dlm" => ewr_Encrypt($fld->FldDelimiter));
			$sSqlWrk = "";
		$this->Lookup_Selecting($this->group, $sWhereWrk); // Call Lookup selecting
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
		$sSqlWrk .= " ORDER BY `group` ASC";
			if ($sSqlWrk <> "")
