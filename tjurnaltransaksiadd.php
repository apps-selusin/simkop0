...
$tjurnaltransaksi_add = NULL; // Initialize page object first

class ctjurnaltransaksi_add extends ctjurnaltransaksi {
	...
	// Page object name
	var $PageObjName = 'tjurnaltransaksi_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tjurnaltransaksiview.php")
	...
	function LoadDefaultValues() {
		...
		$this->pembayaran_->CurrentValue = 0;
		$this->bunga_->CurrentValue = 0;
		$this->denda_->CurrentValue = 0;
		$this->titipan_->CurrentValue = 0;
		$this->administrasi_->CurrentValue = 0;
		$this->modal_->CurrentValue = 0;
		$this->pinjaman_->CurrentValue = 0;
		$this->biaya_->CurrentValue = 0;
	...
	function AddRow($rsold = NULL) {
		...
		// pembayaran_
		$this->pembayaran_->SetDbValueDef($rsnew, $this->pembayaran_->CurrentValue, 0, strval($this->pembayaran_->CurrentValue) == "");

		// bunga_
		$this->bunga_->SetDbValueDef($rsnew, $this->bunga_->CurrentValue, 0, strval($this->bunga_->CurrentValue) == "");

		// denda_
		$this->denda_->SetDbValueDef($rsnew, $this->denda_->CurrentValue, 0, strval($this->denda_->CurrentValue) == "");

		// titipan_
		$this->titipan_->SetDbValueDef($rsnew, $this->titipan_->CurrentValue, 0, strval($this->titipan_->CurrentValue) == "");

		// administrasi_
		$this->administrasi_->SetDbValueDef($rsnew, $this->administrasi_->CurrentValue, 0, strval($this->administrasi_->CurrentValue) == "");

		// modal_
		$this->modal_->SetDbValueDef($rsnew, $this->modal_->CurrentValue, 0, strval($this->modal_->CurrentValue) == "");

		// pinjaman_
		$this->pinjaman_->SetDbValueDef($rsnew, $this->pinjaman_->CurrentValue, 0, strval($this->pinjaman_->CurrentValue) == "");

		// biaya_
		$this->biaya_->SetDbValueDef($rsnew, $this->biaya_->CurrentValue, 0, strval($this->biaya_->CurrentValue) == "");
...
// Create page object
if (!isset($tjurnaltransaksi_add)) $tjurnaltransaksi_add = new ctjurnaltransaksi_add();

// Page init
$tjurnaltransaksi_add->Page_Init();

// Page main
$tjurnaltransaksi_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tjurnaltransaksi_add->Page_Render();
?>
...
var CurrentForm = ftjurnaltransaksiadd = new ew_Form("ftjurnaltransaksiadd", "add");

// Validate form
ftjurnaltransaksiadd.Validate = function() {
...
ftjurnaltransaksiadd.Form_CustomValidate = 
...
ftjurnaltransaksiadd.ValidateRequired = true;
<?php } else { ?>
ftjurnaltransaksiadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftjurnaltransaksiadd.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftjurnaltransaksiadd.Lists["x_active"].Options = <?php echo json_encode($tjurnaltransaksi->active->Options()) ?>;
...
<?php if (!$tjurnaltransaksi_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $tjurnaltransaksi_add->ShowPageHeader(); ?>
<?php
$tjurnaltransaksi_add->ShowMessage();
?>
<form name="ftjurnaltransaksiadd" id="ftjurnaltransaksiadd" class="<?php echo $tjurnaltransaksi_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tjurnaltransaksi_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tjurnaltransaksi_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="tjurnaltransaksi" data-field="x_id" name="x_id" id="x_id" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tjurnaltransaksi->id->getPlaceHolder()) ?>" value="<?php echo $tjurnaltransaksi->id->EditValue ?>"<?php echo $tjurnaltransaksi->id->EditAttributes() ?>>
</span>
...
<input type="text" data-table="tjurnaltransaksi" data-field="x_nomor" name="x_nomor" id="x_nomor" size="30" placeholder="<?php echo ew_HtmlEncode($tjurnaltransaksi->nomor->getPlaceHolder()) ?>" value="<?php echo $tjurnaltransaksi->nomor->EditValue ?>"<?php echo $tjurnaltransaksi->nomor->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tjurnaltransaksi_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ftjurnaltransaksiadd.Init();
</script>
<?php
$tjurnaltransaksi_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tjurnaltransaksi_add->Page_Terminate();
?>
