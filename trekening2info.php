...
//
// Table class for trekening2
//
class ctrekening2 extends cTable {
	var $group;
	var $id;
	var $rekening;
	var $parent;
	...
	function __construct() {
		...
		$this->TableVar = 'trekening2';
		$this->TableName = 'trekening2';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`trekening2`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// group
		$this->group = new cField('trekening2', 'trekening2', 'x_group', 'group', '`group`', '`group`', 20, -1, FALSE, '`group`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// id
		$this->id = new cField('trekening2', 'trekening2', 'x_id', 'id', '`id`', '`id`', 200, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id->Sortable = TRUE; // Allow sort
		$this->fields['id'] = &$this->id;

		// rekening
		$this->rekening = new cField('trekening2', 'trekening2', 'x_rekening', 'rekening', '`rekening`', '`rekening`', 200, -1, FALSE, '`rekening`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rekening->Sortable = TRUE; // Allow sort
		$this->fields['rekening'] = &$this->rekening;

		// parent
		$this->parent = new cField('trekening2', 'trekening2', 'x_parent', 'parent', '`parent`', '`parent`', 200, -1, FALSE, '`parent`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// parent1
		$this->parent1 = new cField('trekening2', 'trekening2', 'x_parent1', 'parent1', '(CASE
 WHEN (LENGTH(parent) = 1) THEN parent ELSE \'\' END)', '(CASE
 WHEN (LENGTH(parent) = 1) THEN parent ELSE \'\' END)', 200, -1, FALSE, '(CASE
 WHEN (LENGTH(parent) = 1) THEN parent ELSE \'\' END)', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->parent1->FldIsCustom = TRUE; // Custom field
		$this->parent1->Sortable = TRUE; // Allow sort
		$this->fields['parent1'] = &$this->parent1;

		// id1
		$this->id1 = new cField('trekening2', 'trekening2', 'x_id1', 'id1', '(CASE
 WHEN (LENGTH(parent) = 1) THEN id ELSE \'\' END)', '(CASE
 WHEN (LENGTH(parent) = 1) THEN id ELSE \'\' END)', 200, -1, FALSE, '(CASE
 WHEN (LENGTH(parent) = 1) THEN id ELSE \'\' END)', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id1->FldIsCustom = TRUE; // Custom field
		$this->id1->Sortable = TRUE; // Allow sort
		$this->fields['id1'] = &$this->id1;

		// rekening1
		$this->rekening1 = new cField('trekening2', 'trekening2', 'x_rekening1', 'rekening1', '(CASE
 WHEN (LENGTH(parent) = 1) THEN rekening ELSE \'\' END)', '(CASE
 WHEN (LENGTH(parent) = 1) THEN rekening ELSE \'\' END)', 200, -1, FALSE, '(CASE
 WHEN (LENGTH(parent) = 1) THEN rekening ELSE \'\' END)', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rekening1->FldIsCustom = TRUE; // Custom field
		$this->rekening1->Sortable = TRUE; // Allow sort
		$this->fields['rekening1'] = &$this->rekening1;

		// parent2
		$this->parent2 = new cField('trekening2', 'trekening2', 'x_parent2', 'parent2', '(CASE
 WHEN (LENGTH(parent) > 1) THEN parent ELSE \'\' END)', '(CASE
 WHEN (LENGTH(parent) > 1) THEN parent ELSE \'\' END)', 200, -1, FALSE, '(CASE
 WHEN (LENGTH(parent) > 1) THEN parent ELSE \'\' END)', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->parent2->FldIsCustom = TRUE; // Custom field
		$this->parent2->Sortable = TRUE; // Allow sort
		$this->fields['parent2'] = &$this->parent2;

		// id2
		$this->id2 = new cField('trekening2', 'trekening2', 'x_id2', 'id2', '(CASE
 WHEN (LENGTH(parent) > 1) THEN id ELSE \'\' END)', '(CASE
 WHEN (LENGTH(parent) > 1) THEN id ELSE \'\' END)', 200, -1, FALSE, '(CASE
 WHEN (LENGTH(parent) > 1) THEN id ELSE \'\' END)', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// rekening2
		$this->rekening2 = new cField('trekening2', 'trekening2', 'x_rekening2', 'rekening2', '(CASE
 WHEN (LENGTH(parent) > 1) THEN rekening ELSE \'\' END)', '(CASE
 WHEN (LENGTH(parent) > 1) THEN rekening ELSE \'\' END)', 200, -1, FALSE, '(CASE
 WHEN (LENGTH(parent) > 1) THEN rekening ELSE \'\' END)', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->rekening2->FldIsCustom = TRUE; // Custom field
		$this->rekening2->Sortable = TRUE; // Allow sort
		$this->fields['rekening2'] = &$this->rekening2;

		// tipe
		$this->tipe = new cField('trekening2', 'trekening2', 'x_tipe', 'tipe', '`tipe`', '`tipe`', 200, -1, FALSE, '`tipe`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->tipe->Sortable = TRUE; // Allow sort
		$this->fields['tipe'] = &$this->tipe;

		// posisi
		$this->posisi = new cField('trekening2', 'trekening2', 'x_posisi', 'posisi', '`posisi`', '`posisi`', 200, -1, FALSE, '`posisi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->posisi->Sortable = TRUE; // Allow sort
		$this->fields['posisi'] = &$this->posisi;

		// laporan
		$this->laporan = new cField('trekening2', 'trekening2', 'x_laporan', 'laporan', '`laporan`', '`laporan`', 200, -1, FALSE, '`laporan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->laporan->Sortable = TRUE; // Allow sort
		$this->fields['laporan'] = &$this->laporan;

		// status
		$this->status = new cField('trekening2', 'trekening2', 'x_status', 'status', '`status`', '`status`', 200, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->status->Sortable = TRUE; // Allow sort
		$this->fields['status'] = &$this->status;

		// keterangan
		$this->keterangan = new cField('trekening2', 'trekening2', 'x_keterangan', 'keterangan', '`keterangan`', '`keterangan`', 200, -1, FALSE, '`keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->keterangan->Sortable = TRUE; // Allow sort
		$this->fields['keterangan'] = &$this->keterangan;

		// active
		$this->active = new cField('trekening2', 'trekening2', 'x_active', 'active', '`active`', '`active`', 202, -1, FALSE, '`active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`trekening2`";
	}
	...
	function getSqlSelect() { // Select
		return ($this->_SqlSelect <> "") ? $this->_SqlSelect : "SELECT *, (CASE
 WHEN (LENGTH(parent) = 1) THEN parent ELSE '' END) AS `parent1`, (CASE
 WHEN (LENGTH(parent) = 1) THEN id ELSE '' END) AS `id1`, (CASE
 WHEN (LENGTH(parent) = 1) THEN rekening ELSE '' END) AS `rekening1`, (CASE
 WHEN (LENGTH(parent) > 1) THEN parent ELSE '' END) AS `parent2`, (CASE
 WHEN (LENGTH(parent) > 1) THEN id ELSE '' END) AS `id2`, (CASE
 WHEN (LENGTH(parent) > 1) THEN rekening ELSE '' END) AS `rekening2` FROM " . $this->getSqlFrom();
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("trekening2view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("trekening2view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "trekening2add.php?" . $this->UrlParm($parm);
		else
			$url = "trekening2add.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("trekening2edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("trekening2add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("trekening2delete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->parent1->EditValue = $this->parent1->CurrentValue;
		...
		$this->parent2->EditValue = $this->parent2->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->parent1->Exportable) $Doc->ExportCaption($this->parent1);
					if ($this->id1->Exportable) $Doc->ExportCaption($this->id1);
					if ($this->rekening1->Exportable) $Doc->ExportCaption($this->rekening1);
					if ($this->parent2->Exportable) $Doc->ExportCaption($this->parent2);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->parent1->Exportable) $Doc->ExportField($this->parent1);
						if ($this->id1->Exportable) $Doc->ExportField($this->id1);
						if ($this->rekening1->Exportable) $Doc->ExportField($this->rekening1);
						if ($this->parent2->Exportable) $Doc->ExportField($this->parent2);
