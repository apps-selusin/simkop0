...
$tjurnal_list = NULL; // Initialize page object first

class ctjurnal_list extends ctjurnal {
	...
	// Page object name
	var $PageObjName = 'tjurnal_list';

	// Grid form hidden field names
	var $FormName = 'ftjurnallist';
	...
	function __construct() {
		...
		$this->AddUrl = "tjurnaladd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "tjurnaldelete.php";
		$this->MultiUpdateUrl = "tjurnalupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ftjurnallistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "ftjurnallistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "ftjurnallistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ftjurnallistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ftjurnallistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ftjurnallist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"ftjurnallistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($tjurnal_list)) $tjurnal_list = new ctjurnal_list();

// Page init
$tjurnal_list->Page_Init();

// Page main
$tjurnal_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tjurnal_list->Page_Render();
?>
...
var CurrentForm = ftjurnallist = new ew_Form("ftjurnallist", "list");
ftjurnallist.FormKeyCountName = '<?php echo $tjurnal_list->FormKeyCountName ?>';

// Form_CustomValidate event
ftjurnallist.Form_CustomValidate = 
...
ftjurnallist.ValidateRequired = true;
<?php } else { ?>
ftjurnallist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftjurnallist.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftjurnallist.Lists["x_active"].Options = <?php echo json_encode($tjurnal->active->Options()) ?>;

// Form object for search
var CurrentSearchForm = ftjurnallistsrch = new ew_Form("ftjurnallistsrch");

// Validate function for search
ftjurnallistsrch.Validate = function(fobj) {
...
// Form_CustomValidate event
ftjurnallistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
<?php if (EW_CLIENT_VALIDATE) { ?>
ftjurnallistsrch.ValidateRequired = true; // Use JavaScript validation
<?php } else { ?>
ftjurnallistsrch.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Dynamic selection lists
ftjurnallistsrch.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftjurnallistsrch.Lists["x_active"].Options = <?php echo json_encode($tjurnal->active->Options()) ?>;
</script>
<script type="text/javascript">
...
<?php if ($tjurnal_list->TotalRecs > 0 && $tjurnal_list->ExportOptions->Visible()) { ?>
<?php $tjurnal_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($tjurnal_list->SearchOptions->Visible()) { ?>
<?php $tjurnal_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($tjurnal_list->FilterOptions->Visible()) { ?>
<?php $tjurnal_list->FilterOptions->Render("body") ?>
<?php } ?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php
	$bSelectLimit = $tjurnal_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($tjurnal_list->TotalRecs <= 0)
			$tjurnal_list->TotalRecs = $tjurnal->SelectRecordCount();
	} else {
		if (!$tjurnal_list->Recordset && ($tjurnal_list->Recordset = $tjurnal_list->LoadRecordset()))
			$tjurnal_list->TotalRecs = $tjurnal_list->Recordset->RecordCount();
	}
	$tjurnal_list->StartRec = 1;
	if ($tjurnal_list->DisplayRecs <= 0 || ($tjurnal->Export <> "" && $tjurnal->ExportAll)) // Display all records
		$tjurnal_list->DisplayRecs = $tjurnal_list->TotalRecs;
	if (!($tjurnal->Export <> "" && $tjurnal->ExportAll))
		$tjurnal_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$tjurnal_list->Recordset = $tjurnal_list->LoadRecordset($tjurnal_list->StartRec-1, $tjurnal_list->DisplayRecs);

	// Set no record found message
	if ($tjurnal->CurrentAction == "" && $tjurnal_list->TotalRecs == 0) {
		if ($tjurnal_list->SearchWhere == "0=101")
			$tjurnal_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$tjurnal_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$tjurnal_list->RenderOtherOptions();
?>
<?php if ($tjurnal->Export == "" && $tjurnal->CurrentAction == "") { ?>
<form name="ftjurnallistsrch" id="ftjurnallistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($tjurnal_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="ftjurnallistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="tjurnal">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$tjurnal_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$tjurnal->RowType = EW_ROWTYPE_SEARCH;

// Render row
$tjurnal->ResetAttrs();
$tjurnal_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($tjurnal->active->Visible) { // active ?>
	<div id="xsc_active" class="ewCell form-group">
		<label class="ewSearchCaption ewLabel"><?php echo $tjurnal->active->FldCaption() ?></label>
...
<div id="xsr_2" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($tjurnal_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($tjurnal_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $tjurnal_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($tjurnal_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($tjurnal_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($tjurnal_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($tjurnal_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php $tjurnal_list->ShowPageHeader(); ?>
<?php
$tjurnal_list->ShowMessage();
?>
<?php if ($tjurnal_list->TotalRecs > 0 || $tjurnal->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid tjurnal">
<form name="ftjurnallist" id="ftjurnallist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tjurnal_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tjurnal_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tjurnal">
<div id="gmp_tjurnal" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($tjurnal_list->TotalRecs > 0 || $tjurnal->CurrentAction == "gridedit") { ?>
<table id="tbl_tjurnallist" class="table ewTable">
...
// Header row
$tjurnal_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$tjurnal_list->RenderListOptions();

// Render list options (header, left)
$tjurnal_list->ListOptions->Render("header", "left");
?>
<?php if ($tjurnal->tanggal->Visible) { // tanggal ?>
	<?php if ($tjurnal->SortUrl($tjurnal->tanggal) == "") { ?>
		<th data-name="tanggal"><div id="elh_tjurnal_tanggal" class="tjurnal_tanggal"><div class="ewTableHeaderCaption"><?php echo $tjurnal->tanggal->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="tanggal"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->tanggal) ?>',1);"><div id="elh_tjurnal_tanggal" class="tjurnal_tanggal">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->tanggal->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->tanggal->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->tanggal->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->periode->Visible) { // periode ?>
	<?php if ($tjurnal->SortUrl($tjurnal->periode) == "") { ?>
		<th data-name="periode"><div id="elh_tjurnal_periode" class="tjurnal_periode"><div class="ewTableHeaderCaption"><?php echo $tjurnal->periode->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="periode"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->periode) ?>',1);"><div id="elh_tjurnal_periode" class="tjurnal_periode">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->periode->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->periode->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->periode->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->id->Visible) { // id ?>
	<?php if ($tjurnal->SortUrl($tjurnal->id) == "") { ?>
		<th data-name="id"><div id="elh_tjurnal_id" class="tjurnal_id"><div class="ewTableHeaderCaption"><?php echo $tjurnal->id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->id) ?>',1);"><div id="elh_tjurnal_id" class="tjurnal_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->nomor->Visible) { // nomor ?>
	<?php if ($tjurnal->SortUrl($tjurnal->nomor) == "") { ?>
		<th data-name="nomor"><div id="elh_tjurnal_nomor" class="tjurnal_nomor"><div class="ewTableHeaderCaption"><?php echo $tjurnal->nomor->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nomor"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->nomor) ?>',1);"><div id="elh_tjurnal_nomor" class="tjurnal_nomor">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->nomor->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->nomor->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->nomor->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->transaksi->Visible) { // transaksi ?>
	<?php if ($tjurnal->SortUrl($tjurnal->transaksi) == "") { ?>
		<th data-name="transaksi"><div id="elh_tjurnal_transaksi" class="tjurnal_transaksi"><div class="ewTableHeaderCaption"><?php echo $tjurnal->transaksi->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="transaksi"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->transaksi) ?>',1);"><div id="elh_tjurnal_transaksi" class="tjurnal_transaksi">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->transaksi->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->transaksi->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->transaksi->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->referensi->Visible) { // referensi ?>
	<?php if ($tjurnal->SortUrl($tjurnal->referensi) == "") { ?>
		<th data-name="referensi"><div id="elh_tjurnal_referensi" class="tjurnal_referensi"><div class="ewTableHeaderCaption"><?php echo $tjurnal->referensi->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="referensi"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->referensi) ?>',1);"><div id="elh_tjurnal_referensi" class="tjurnal_referensi">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->referensi->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->referensi->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->referensi->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->model->Visible) { // model ?>
	<?php if ($tjurnal->SortUrl($tjurnal->model) == "") { ?>
		<th data-name="model"><div id="elh_tjurnal_model" class="tjurnal_model"><div class="ewTableHeaderCaption"><?php echo $tjurnal->model->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="model"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->model) ?>',1);"><div id="elh_tjurnal_model" class="tjurnal_model">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->model->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->model->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->model->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->rekening->Visible) { // rekening ?>
	<?php if ($tjurnal->SortUrl($tjurnal->rekening) == "") { ?>
		<th data-name="rekening"><div id="elh_tjurnal_rekening" class="tjurnal_rekening"><div class="ewTableHeaderCaption"><?php echo $tjurnal->rekening->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="rekening"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->rekening) ?>',1);"><div id="elh_tjurnal_rekening" class="tjurnal_rekening">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->rekening->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->rekening->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->rekening->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->tipe->Visible) { // tipe ?>
	<?php if ($tjurnal->SortUrl($tjurnal->tipe) == "") { ?>
		<th data-name="tipe"><div id="elh_tjurnal_tipe" class="tjurnal_tipe"><div class="ewTableHeaderCaption"><?php echo $tjurnal->tipe->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="tipe"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->tipe) ?>',1);"><div id="elh_tjurnal_tipe" class="tjurnal_tipe">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->tipe->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->tipe->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->tipe->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->posisi->Visible) { // posisi ?>
	<?php if ($tjurnal->SortUrl($tjurnal->posisi) == "") { ?>
		<th data-name="posisi"><div id="elh_tjurnal_posisi" class="tjurnal_posisi"><div class="ewTableHeaderCaption"><?php echo $tjurnal->posisi->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="posisi"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->posisi) ?>',1);"><div id="elh_tjurnal_posisi" class="tjurnal_posisi">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->posisi->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->posisi->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->posisi->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->laporan->Visible) { // laporan ?>
	<?php if ($tjurnal->SortUrl($tjurnal->laporan) == "") { ?>
		<th data-name="laporan"><div id="elh_tjurnal_laporan" class="tjurnal_laporan"><div class="ewTableHeaderCaption"><?php echo $tjurnal->laporan->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="laporan"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->laporan) ?>',1);"><div id="elh_tjurnal_laporan" class="tjurnal_laporan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->laporan->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->laporan->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->laporan->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->debet->Visible) { // debet ?>
	<?php if ($tjurnal->SortUrl($tjurnal->debet) == "") { ?>
		<th data-name="debet"><div id="elh_tjurnal_debet" class="tjurnal_debet"><div class="ewTableHeaderCaption"><?php echo $tjurnal->debet->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="debet"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->debet) ?>',1);"><div id="elh_tjurnal_debet" class="tjurnal_debet">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->debet->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->debet->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->debet->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->credit->Visible) { // credit ?>
	<?php if ($tjurnal->SortUrl($tjurnal->credit) == "") { ?>
		<th data-name="credit"><div id="elh_tjurnal_credit" class="tjurnal_credit"><div class="ewTableHeaderCaption"><?php echo $tjurnal->credit->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="credit"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->credit) ?>',1);"><div id="elh_tjurnal_credit" class="tjurnal_credit">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->credit->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->credit->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->credit->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->kantor->Visible) { // kantor ?>
	<?php if ($tjurnal->SortUrl($tjurnal->kantor) == "") { ?>
		<th data-name="kantor"><div id="elh_tjurnal_kantor" class="tjurnal_kantor"><div class="ewTableHeaderCaption"><?php echo $tjurnal->kantor->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="kantor"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->kantor) ?>',1);"><div id="elh_tjurnal_kantor" class="tjurnal_kantor">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->kantor->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->kantor->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->kantor->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->keterangan->Visible) { // keterangan ?>
	<?php if ($tjurnal->SortUrl($tjurnal->keterangan) == "") { ?>
		<th data-name="keterangan"><div id="elh_tjurnal_keterangan" class="tjurnal_keterangan"><div class="ewTableHeaderCaption"><?php echo $tjurnal->keterangan->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="keterangan"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->keterangan) ?>',1);"><div id="elh_tjurnal_keterangan" class="tjurnal_keterangan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->keterangan->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->keterangan->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->keterangan->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->active->Visible) { // active ?>
	<?php if ($tjurnal->SortUrl($tjurnal->active) == "") { ?>
		<th data-name="active"><div id="elh_tjurnal_active" class="tjurnal_active"><div class="ewTableHeaderCaption"><?php echo $tjurnal->active->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="active"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->active) ?>',1);"><div id="elh_tjurnal_active" class="tjurnal_active">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->active->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->active->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->active->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->ip->Visible) { // ip ?>
	<?php if ($tjurnal->SortUrl($tjurnal->ip) == "") { ?>
		<th data-name="ip"><div id="elh_tjurnal_ip" class="tjurnal_ip"><div class="ewTableHeaderCaption"><?php echo $tjurnal->ip->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="ip"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->ip) ?>',1);"><div id="elh_tjurnal_ip" class="tjurnal_ip">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->ip->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->ip->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->ip->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->status->Visible) { // status ?>
	<?php if ($tjurnal->SortUrl($tjurnal->status) == "") { ?>
		<th data-name="status"><div id="elh_tjurnal_status" class="tjurnal_status"><div class="ewTableHeaderCaption"><?php echo $tjurnal->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->status) ?>',1);"><div id="elh_tjurnal_status" class="tjurnal_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->status->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->user->Visible) { // user ?>
	<?php if ($tjurnal->SortUrl($tjurnal->user) == "") { ?>
		<th data-name="user"><div id="elh_tjurnal_user" class="tjurnal_user"><div class="ewTableHeaderCaption"><?php echo $tjurnal->user->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="user"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->user) ?>',1);"><div id="elh_tjurnal_user" class="tjurnal_user">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->user->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->user->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->user->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->created->Visible) { // created ?>
	<?php if ($tjurnal->SortUrl($tjurnal->created) == "") { ?>
		<th data-name="created"><div id="elh_tjurnal_created" class="tjurnal_created"><div class="ewTableHeaderCaption"><?php echo $tjurnal->created->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="created"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->created) ?>',1);"><div id="elh_tjurnal_created" class="tjurnal_created">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->created->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->created->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->created->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tjurnal->modified->Visible) { // modified ?>
	<?php if ($tjurnal->SortUrl($tjurnal->modified) == "") { ?>
		<th data-name="modified"><div id="elh_tjurnal_modified" class="tjurnal_modified"><div class="ewTableHeaderCaption"><?php echo $tjurnal->modified->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="modified"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tjurnal->SortUrl($tjurnal->modified) ?>',1);"><div id="elh_tjurnal_modified" class="tjurnal_modified">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tjurnal->modified->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($tjurnal->modified->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tjurnal->modified->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
$tjurnal_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($tjurnal->ExportAll && $tjurnal->Export <> "") {
	$tjurnal_list->StopRec = $tjurnal_list->TotalRecs;
} else {

	// Set the last record to display
	if ($tjurnal_list->TotalRecs > $tjurnal_list->StartRec + $tjurnal_list->DisplayRecs - 1)
		$tjurnal_list->StopRec = $tjurnal_list->StartRec + $tjurnal_list->DisplayRecs - 1;
	else
		$tjurnal_list->StopRec = $tjurnal_list->TotalRecs;
}
$tjurnal_list->RecCnt = $tjurnal_list->StartRec - 1;
if ($tjurnal_list->Recordset && !$tjurnal_list->Recordset->EOF) {
	$tjurnal_list->Recordset->MoveFirst();
	$bSelectLimit = $tjurnal_list->UseSelectLimit;
	if (!$bSelectLimit && $tjurnal_list->StartRec > 1)
		$tjurnal_list->Recordset->Move($tjurnal_list->StartRec - 1);
} elseif (!$tjurnal->AllowAddDeleteRow && $tjurnal_list->StopRec == 0) {
	$tjurnal_list->StopRec = $tjurnal->GridAddRowCount;
}

// Initialize aggregate
$tjurnal->RowType = EW_ROWTYPE_AGGREGATEINIT;
$tjurnal->ResetAttrs();
$tjurnal_list->RenderRow();
while ($tjurnal_list->RecCnt < $tjurnal_list->StopRec) {
	$tjurnal_list->RecCnt++;
	if (intval($tjurnal_list->RecCnt) >= intval($tjurnal_list->StartRec)) {
		$tjurnal_list->RowCnt++;

		// Set up key count
		$tjurnal_list->KeyCount = $tjurnal_list->RowIndex;

		// Init row class and style
		$tjurnal->ResetAttrs();
		$tjurnal->CssClass = "";
		if ($tjurnal->CurrentAction == "gridadd") {
		} else {
			$tjurnal_list->LoadRowValues($tjurnal_list->Recordset); // Load row values
		}
		$tjurnal->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$tjurnal->RowAttrs = array_merge($tjurnal->RowAttrs, array('data-rowindex'=>$tjurnal_list->RowCnt, 'id'=>'r' . $tjurnal_list->RowCnt . '_tjurnal', 'data-rowtype'=>$tjurnal->RowType));
...
$tjurnal_list->ListOptions->Render("body", "left", $tjurnal_list->RowCnt);
?>
	<?php if ($tjurnal->tanggal->Visible) { // tanggal ?>
		<td data-name="tanggal"<?php echo $tjurnal->tanggal->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_tanggal" class="tjurnal_tanggal">
<span<?php echo $tjurnal->tanggal->ViewAttributes() ?>>
<?php echo $tjurnal->tanggal->ListViewValue() ?></span>
</span>
<a id="<?php echo $tjurnal_list->PageObjName . "_row_" . $tjurnal_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($tjurnal->periode->Visible) { // periode ?>
		<td data-name="periode"<?php echo $tjurnal->periode->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_periode" class="tjurnal_periode">
...
<?php echo $tjurnal->periode->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->id->Visible) { // id ?>
		<td data-name="id"<?php echo $tjurnal->id->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_id" class="tjurnal_id">
...
<?php echo $tjurnal->id->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->nomor->Visible) { // nomor ?>
		<td data-name="nomor"<?php echo $tjurnal->nomor->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_nomor" class="tjurnal_nomor">
...
<?php echo $tjurnal->nomor->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->transaksi->Visible) { // transaksi ?>
		<td data-name="transaksi"<?php echo $tjurnal->transaksi->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_transaksi" class="tjurnal_transaksi">
...
<?php echo $tjurnal->transaksi->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->referensi->Visible) { // referensi ?>
		<td data-name="referensi"<?php echo $tjurnal->referensi->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_referensi" class="tjurnal_referensi">
...
<?php echo $tjurnal->referensi->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->model->Visible) { // model ?>
		<td data-name="model"<?php echo $tjurnal->model->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_model" class="tjurnal_model">
...
<?php echo $tjurnal->model->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->rekening->Visible) { // rekening ?>
		<td data-name="rekening"<?php echo $tjurnal->rekening->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_rekening" class="tjurnal_rekening">
...
<?php echo $tjurnal->rekening->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->tipe->Visible) { // tipe ?>
		<td data-name="tipe"<?php echo $tjurnal->tipe->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_tipe" class="tjurnal_tipe">
...
<?php echo $tjurnal->tipe->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->posisi->Visible) { // posisi ?>
		<td data-name="posisi"<?php echo $tjurnal->posisi->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_posisi" class="tjurnal_posisi">
...
<?php echo $tjurnal->posisi->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->laporan->Visible) { // laporan ?>
		<td data-name="laporan"<?php echo $tjurnal->laporan->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_laporan" class="tjurnal_laporan">
...
<?php echo $tjurnal->laporan->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->debet->Visible) { // debet ?>
		<td data-name="debet"<?php echo $tjurnal->debet->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_debet" class="tjurnal_debet">
...
<?php echo $tjurnal->debet->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->credit->Visible) { // credit ?>
		<td data-name="credit"<?php echo $tjurnal->credit->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_credit" class="tjurnal_credit">
...
<?php echo $tjurnal->credit->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->kantor->Visible) { // kantor ?>
		<td data-name="kantor"<?php echo $tjurnal->kantor->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_kantor" class="tjurnal_kantor">
...
<?php echo $tjurnal->kantor->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->keterangan->Visible) { // keterangan ?>
		<td data-name="keterangan"<?php echo $tjurnal->keterangan->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_keterangan" class="tjurnal_keterangan">
...
<?php echo $tjurnal->keterangan->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->active->Visible) { // active ?>
		<td data-name="active"<?php echo $tjurnal->active->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_active" class="tjurnal_active">
...
<?php echo $tjurnal->active->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->ip->Visible) { // ip ?>
		<td data-name="ip"<?php echo $tjurnal->ip->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_ip" class="tjurnal_ip">
...
<?php echo $tjurnal->ip->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->status->Visible) { // status ?>
		<td data-name="status"<?php echo $tjurnal->status->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_status" class="tjurnal_status">
...
<?php echo $tjurnal->status->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->user->Visible) { // user ?>
		<td data-name="user"<?php echo $tjurnal->user->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_user" class="tjurnal_user">
...
<?php echo $tjurnal->user->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->created->Visible) { // created ?>
		<td data-name="created"<?php echo $tjurnal->created->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_created" class="tjurnal_created">
...
<?php echo $tjurnal->created->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($tjurnal->modified->Visible) { // modified ?>
		<td data-name="modified"<?php echo $tjurnal->modified->CellAttributes() ?>>
<span id="el<?php echo $tjurnal_list->RowCnt ?>_tjurnal_modified" class="tjurnal_modified">
...
$tjurnal_list->ListOptions->Render("body", "right", $tjurnal_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($tjurnal->CurrentAction <> "gridadd")
		$tjurnal_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($tjurnal->CurrentAction == "") { ?>
...
// Close recordset
if ($tjurnal_list->Recordset)
	$tjurnal_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($tjurnal->CurrentAction <> "gridadd" && $tjurnal->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($tjurnal_list->Pager)) $tjurnal_list->Pager = new cPrevNextPager($tjurnal_list->StartRec, $tjurnal_list->DisplayRecs, $tjurnal_list->TotalRecs) ?>
<?php if ($tjurnal_list->Pager->RecordCount > 0 && $tjurnal_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($tjurnal_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $tjurnal_list->PageUrl() ?>start=<?php echo $tjurnal_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($tjurnal_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $tjurnal_list->PageUrl() ?>start=<?php echo $tjurnal_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $tjurnal_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($tjurnal_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $tjurnal_list->PageUrl() ?>start=<?php echo $tjurnal_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($tjurnal_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $tjurnal_list->PageUrl() ?>start=<?php echo $tjurnal_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $tjurnal_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $tjurnal_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $tjurnal_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $tjurnal_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($tjurnal_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($tjurnal_list->TotalRecs == 0 && $tjurnal->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($tjurnal_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
ftjurnallistsrch.FilterList = <?php echo $tjurnal_list->GetFilterList() ?>;
ftjurnallistsrch.Init();
ftjurnallist.Init();
</script>
<?php
$tjurnal_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
