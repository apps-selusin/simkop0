...
$tpinjamandetail__add = NULL; // Initialize page object first

class ctpinjamandetail__add extends ctpinjamandetail_ {
	...
	// Page object name
	var $PageObjName = 'tpinjamandetail__add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["berjangka"] != "") {
				...
				$this->setKey("berjangka", $this->berjangka->CurrentValue); // Set up key
			} else {
				$this->setKey("berjangka", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if (@$_GET["angsuran"] != "") {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tpinjamandetail_view.php")
	...
	function RenderRow() {
		...
		$this->sisaangsuranauto->ViewCustomAttributes = "";
			...
			$this->berjangka->EditValue = ew_HtmlEncode($this->berjangka->CurrentValue);
			$this->berjangka->PlaceHolder = ew_RemoveHtml($this->berjangka->FldCaption());
	...
	function AddRow($rsold = NULL) {
		...
		// berjangka
		$this->berjangka->SetDbValueDef($rsnew, $this->berjangka->CurrentValue, 0, strval($this->berjangka->CurrentValue) == "");
		...
		// Check if key value entered
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['berjangka']) == "") {
			$this->setFailureMessage($Language->Phrase("InvalidKeyValue"));
			$bInsertRow = FALSE;
		}
...
// Create page object
if (!isset($tpinjamandetail__add)) $tpinjamandetail__add = new ctpinjamandetail__add();

// Page init
$tpinjamandetail__add->Page_Init();

// Page main
$tpinjamandetail__add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tpinjamandetail__add->Page_Render();
?>
...
var CurrentForm = ftpinjamandetail_add = new ew_Form("ftpinjamandetail_add", "add");

// Validate form
ftpinjamandetail_add.Validate = function() {
...
ftpinjamandetail_add.Form_CustomValidate = 
...
ftpinjamandetail_add.ValidateRequired = true;
<?php } else { ?>
ftpinjamandetail_add.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$tpinjamandetail__add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $tpinjamandetail__add->ShowPageHeader(); ?>
<?php
$tpinjamandetail__add->ShowMessage();
?>
<form name="ftpinjamandetail_add" id="ftpinjamandetail_add" class="<?php echo $tpinjamandetail__add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tpinjamandetail__add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tpinjamandetail__add->Token ?>">
<?php } ?>
...
<input type="text" data-table="tpinjamandetail_" data-field="x_id" name="x_id" id="x_id" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tpinjamandetail_->id->getPlaceHolder()) ?>" value="<?php echo $tpinjamandetail_->id->EditValue ?>"<?php echo $tpinjamandetail_->id->EditAttributes() ?>>
</span>
...
<input type="text" data-table="tpinjamandetail_" data-field="x_berjangka" name="x_berjangka" id="x_berjangka" size="30" placeholder="<?php echo ew_HtmlEncode($tpinjamandetail_->berjangka->getPlaceHolder()) ?>" value="<?php echo $tpinjamandetail_->berjangka->EditValue ?>"<?php echo $tpinjamandetail_->berjangka->EditAttributes() ?>>
</span>
...
<input type="text" data-table="tpinjamandetail_" data-field="x_angsuran" name="x_angsuran" id="x_angsuran" size="30" placeholder="<?php echo ew_HtmlEncode($tpinjamandetail_->angsuran->getPlaceHolder()) ?>" value="<?php echo $tpinjamandetail_->angsuran->EditValue ?>"<?php echo $tpinjamandetail_->angsuran->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tpinjamandetail__add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ftpinjamandetail_add.Init();
</script>
<?php
$tpinjamandetail__add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tpinjamandetail__add->Page_Terminate();
?>
