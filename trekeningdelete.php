...
$trekening_delete = NULL; // Initialize page object first

class ctrekening_delete extends ctrekening {
	...
	// Page object name
	var $PageObjName = 'trekening_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("trekeninglist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in trekening class, trekeninginfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("trekeninglist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($trekening_delete)) $trekening_delete = new ctrekening_delete();

// Page init
$trekening_delete->Page_Init();

// Page main
$trekening_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$trekening_delete->Page_Render();
?>
...
var CurrentForm = ftrekeningdelete = new ew_Form("ftrekeningdelete", "delete");

// Form_CustomValidate event
ftrekeningdelete.Form_CustomValidate = 
...
ftrekeningdelete.ValidateRequired = true;
<?php } else { ?>
ftrekeningdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftrekeningdelete.Lists["x_group"] = {"LinkField":"x_group","Ajax":true,"AutoFill":false,"DisplayFields":["x_rekening","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"trekening"};
ftrekeningdelete.Lists["x_tipe[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftrekeningdelete.Lists["x_tipe[]"].Options = <?php echo json_encode($trekening->tipe->Options()) ?>;
ftrekeningdelete.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftrekeningdelete.Lists["x_active"].Options = <?php echo json_encode($trekening->active->Options()) ?>;
...
<?php $trekening_delete->ShowPageHeader(); ?>
<?php
$trekening_delete->ShowMessage();
?>
<form name="ftrekeningdelete" id="ftrekeningdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($trekening_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $trekening_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="trekening">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($trekening_delete->RecKeys as $key) { ?>
...
<?php if ($trekening->group->Visible) { // group ?>
		<th><span id="elh_trekening_group" class="trekening_group"><?php echo $trekening->group->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening->id->Visible) { // id ?>
		<th><span id="elh_trekening_id" class="trekening_id"><?php echo $trekening->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening->rekening->Visible) { // rekening ?>
		<th><span id="elh_trekening_rekening" class="trekening_rekening"><?php echo $trekening->rekening->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening->tipe->Visible) { // tipe ?>
		<th><span id="elh_trekening_tipe" class="trekening_tipe"><?php echo $trekening->tipe->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening->posisi->Visible) { // posisi ?>
		<th><span id="elh_trekening_posisi" class="trekening_posisi"><?php echo $trekening->posisi->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening->laporan->Visible) { // laporan ?>
		<th><span id="elh_trekening_laporan" class="trekening_laporan"><?php echo $trekening->laporan->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening->status->Visible) { // status ?>
		<th><span id="elh_trekening_status" class="trekening_status"><?php echo $trekening->status->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening->parent->Visible) { // parent ?>
		<th><span id="elh_trekening_parent" class="trekening_parent"><?php echo $trekening->parent->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening->keterangan->Visible) { // keterangan ?>
		<th><span id="elh_trekening_keterangan" class="trekening_keterangan"><?php echo $trekening->keterangan->FldCaption() ?></span></th>
<?php } ?>
<?php if ($trekening->active->Visible) { // active ?>
		<th><span id="elh_trekening_active" class="trekening_active"><?php echo $trekening->active->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$trekening_delete->RecCnt = 0;
$i = 0;
while (!$trekening_delete->Recordset->EOF) {
	$trekening_delete->RecCnt++;
	$trekening_delete->RowCnt++;

	// Set row properties
	$trekening->ResetAttrs();
	$trekening->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$trekening_delete->LoadRowValues($trekening_delete->Recordset);

	// Render row
	$trekening_delete->RenderRow();
?>
	<tr<?php echo $trekening->RowAttributes() ?>>
<?php if ($trekening->group->Visible) { // group ?>
		<td<?php echo $trekening->group->CellAttributes() ?>>
<span id="el<?php echo $trekening_delete->RowCnt ?>_trekening_group" class="trekening_group">
...
<span id="el<?php echo $trekening_delete->RowCnt ?>_trekening_id" class="trekening_id">
...
<span id="el<?php echo $trekening_delete->RowCnt ?>_trekening_rekening" class="trekening_rekening">
...
<span id="el<?php echo $trekening_delete->RowCnt ?>_trekening_tipe" class="trekening_tipe">
...
<span id="el<?php echo $trekening_delete->RowCnt ?>_trekening_posisi" class="trekening_posisi">
...
<span id="el<?php echo $trekening_delete->RowCnt ?>_trekening_laporan" class="trekening_laporan">
...
<span id="el<?php echo $trekening_delete->RowCnt ?>_trekening_status" class="trekening_status">
...
<span id="el<?php echo $trekening_delete->RowCnt ?>_trekening_parent" class="trekening_parent">
...
<span id="el<?php echo $trekening_delete->RowCnt ?>_trekening_keterangan" class="trekening_keterangan">
...
<span id="el<?php echo $trekening_delete->RowCnt ?>_trekening_active" class="trekening_active">
...
<?php
	$trekening_delete->Recordset->MoveNext();
}
$trekening_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $trekening_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftrekeningdelete.Init();
</script>
<?php
$trekening_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$trekening_delete->Page_Terminate();
?>
