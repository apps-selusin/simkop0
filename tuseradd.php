...
$tuser_add = NULL; // Initialize page object first

class ctuser_add extends ctuser {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tuserview.php")
...
// Create page object
if (!isset($tuser_add)) $tuser_add = new ctuser_add();
...
var CurrentForm = ftuseradd = new ew_Form("ftuseradd", "add");

// Validate form
ftuseradd.Validate = function() {
...
ftuseradd.ValidateRequired = true;
<?php } else { ?>
ftuseradd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftuseradd.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftuseradd.Lists["x_active"].Options = <?php echo json_encode($tuser->active->Options()) ?>;
...
<?php } ?>
<?php $tuser_add->ShowPageHeader(); ?>
<?php
$tuser_add->ShowMessage();
?>
<form name="ftuseradd" id="ftuseradd" class="<?php echo $tuser_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tuser_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tuser_add->Token ?>">
<?php } ?>
...
<span id="el_tuser_id">
<input type="text" data-table="tuser" data-field="x_id" name="x_id" id="x_id" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tuser->id->getPlaceHolder()) ?>" value="<?php echo $tuser->id->EditValue ?>"<?php echo $tuser->id->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tuser_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
