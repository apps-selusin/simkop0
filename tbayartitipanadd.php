...
$tbayartitipan_add = NULL; // Initialize page object first

class ctbayartitipan_add extends ctbayartitipan {
	...
	// Page object name
	var $PageObjName = 'tbayartitipan_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["titipan"] != "") {
				...
				$this->setKey("titipan", $this->titipan->CurrentValue); // Set up key
			} else {
				$this->setKey("titipan", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tbayartitipanview.php")
	...
	function RenderRow() {
		...
		$this->modified->ViewCustomAttributes = "";
			...
			$this->titipan->EditValue = ew_HtmlEncode($this->titipan->CurrentValue);
	...
	function AddRow($rsold = NULL) {
		...
		// titipan
		$this->titipan->SetDbValueDef($rsnew, $this->titipan->CurrentValue, 0, strval($this->titipan->CurrentValue) == "");
		...
		// Check if key value entered
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['titipan']) == "") {
			$this->setFailureMessage($Language->Phrase("InvalidKeyValue"));
			$bInsertRow = FALSE;
		}
...
// Create page object
if (!isset($tbayartitipan_add)) $tbayartitipan_add = new ctbayartitipan_add();

// Page init
$tbayartitipan_add->Page_Init();

// Page main
$tbayartitipan_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tbayartitipan_add->Page_Render();
?>
...
var CurrentForm = ftbayartitipanadd = new ew_Form("ftbayartitipanadd", "add");

// Validate form
ftbayartitipanadd.Validate = function() {
...
ftbayartitipanadd.Form_CustomValidate = 
...
ftbayartitipanadd.ValidateRequired = true;
<?php } else { ?>
ftbayartitipanadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftbayartitipanadd.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftbayartitipanadd.Lists["x_active"].Options = <?php echo json_encode($tbayartitipan->active->Options()) ?>;
...
<?php if (!$tbayartitipan_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $tbayartitipan_add->ShowPageHeader(); ?>
<?php
$tbayartitipan_add->ShowMessage();
?>
<form name="ftbayartitipanadd" id="ftbayartitipanadd" class="<?php echo $tbayartitipan_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tbayartitipan_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tbayartitipan_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="tbayartitipan" data-field="x_id" name="x_id" id="x_id" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tbayartitipan->id->getPlaceHolder()) ?>" value="<?php echo $tbayartitipan->id->EditValue ?>"<?php echo $tbayartitipan->id->EditAttributes() ?>>
</span>
...
<input type="text" data-table="tbayartitipan" data-field="x_titipan" name="x_titipan" id="x_titipan" size="30" placeholder="<?php echo ew_HtmlEncode($tbayartitipan->titipan->getPlaceHolder()) ?>" value="<?php echo $tbayartitipan->titipan->EditValue ?>"<?php echo $tbayartitipan->titipan->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tbayartitipan_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ftbayartitipanadd.Init();
</script>
<?php
$tbayartitipan_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tbayartitipan_add->Page_Terminate();
?>
