...
$vrekeninggroup_edit = NULL; // Initialize page object first

class cvrekeninggroup_edit extends cvrekeninggroup {
	...
	// Page object name
	var $PageObjName = 'vrekeninggroup_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("vrekeninggrouplist.php"); // Invalid key, return to list
		}
...
// Create page object
if (!isset($vrekeninggroup_edit)) $vrekeninggroup_edit = new cvrekeninggroup_edit();

// Page init
$vrekeninggroup_edit->Page_Init();

// Page main
$vrekeninggroup_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vrekeninggroup_edit->Page_Render();
?>
...
var CurrentForm = fvrekeninggroupedit = new ew_Form("fvrekeninggroupedit", "edit");

// Validate form
fvrekeninggroupedit.Validate = function() {
...
fvrekeninggroupedit.Form_CustomValidate = 
...
fvrekeninggroupedit.ValidateRequired = true;
<?php } else { ?>
fvrekeninggroupedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvrekeninggroupedit.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvrekeninggroupedit.Lists["x_active"].Options = <?php echo json_encode($vrekeninggroup->active->Options()) ?>;
...
<?php if (!$vrekeninggroup_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $vrekeninggroup_edit->ShowPageHeader(); ?>
<?php
$vrekeninggroup_edit->ShowMessage();
?>
<form name="fvrekeninggroupedit" id="fvrekeninggroupedit" class="<?php echo $vrekeninggroup_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vrekeninggroup_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vrekeninggroup_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $vrekeninggroup->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="vrekeninggroup" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($vrekeninggroup->id->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $vrekeninggroup_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fvrekeninggroupedit.Init();
</script>
<?php
$vrekeninggroup_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vrekeninggroup_edit->Page_Terminate();
?>
