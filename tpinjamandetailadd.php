...
$tpinjamandetail_add = NULL; // Initialize page object first

class ctpinjamandetail_add extends ctpinjamandetail {
	...
	// Page object name
	var $PageObjName = 'tpinjamandetail_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tpinjamandetailview.php")
	...
	function LoadDefaultValues() {
		...
		$this->tanggalbayar->CurrentValue = "0000-00-00 00:00:00";
	...
	function AddRow($rsold = NULL) {
		...
		// tanggalbayar
		$this->tanggalbayar->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->tanggalbayar->CurrentValue, 0), ew_CurrentDate(), strval($this->tanggalbayar->CurrentValue) == "");
...
// Create page object
if (!isset($tpinjamandetail_add)) $tpinjamandetail_add = new ctpinjamandetail_add();

// Page init
$tpinjamandetail_add->Page_Init();

// Page main
$tpinjamandetail_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tpinjamandetail_add->Page_Render();
?>
...
var CurrentForm = ftpinjamandetailadd = new ew_Form("ftpinjamandetailadd", "add");

// Validate form
ftpinjamandetailadd.Validate = function() {
...
ftpinjamandetailadd.Form_CustomValidate = 
...
ftpinjamandetailadd.ValidateRequired = true;
<?php } else { ?>
ftpinjamandetailadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$tpinjamandetail_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $tpinjamandetail_add->ShowPageHeader(); ?>
<?php
$tpinjamandetail_add->ShowMessage();
?>
<form name="ftpinjamandetailadd" id="ftpinjamandetailadd" class="<?php echo $tpinjamandetail_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tpinjamandetail_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tpinjamandetail_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="tpinjamandetail" data-field="x_id" name="x_id" id="x_id" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tpinjamandetail->id->getPlaceHolder()) ?>" value="<?php echo $tpinjamandetail->id->EditValue ?>"<?php echo $tpinjamandetail->id->EditAttributes() ?>>
</span>
...
<input type="text" data-table="tpinjamandetail" data-field="x_angsuran" name="x_angsuran" id="x_angsuran" size="30" placeholder="<?php echo ew_HtmlEncode($tpinjamandetail->angsuran->getPlaceHolder()) ?>" value="<?php echo $tpinjamandetail->angsuran->EditValue ?>"<?php echo $tpinjamandetail->angsuran->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tpinjamandetail_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ftpinjamandetailadd.Init();
</script>
<?php
$tpinjamandetail_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tpinjamandetail_add->Page_Terminate();
?>
