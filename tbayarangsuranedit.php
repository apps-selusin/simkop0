...
$tbayarangsuran_edit = NULL; // Initialize page object first

class ctbayarangsuran_edit extends ctbayarangsuran {
	...
	// Page object name
	var $PageObjName = 'tbayarangsuran_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("tbayarangsuranlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// saldotitipan
			$this->saldotitipan->SetDbValueDef($rsnew, $this->saldotitipan->CurrentValue, 0, $this->saldotitipan->ReadOnly);

			// saldotitipansisa
			$this->saldotitipansisa->SetDbValueDef($rsnew, $this->saldotitipansisa->CurrentValue, 0, $this->saldotitipansisa->ReadOnly);
...
// Create page object
if (!isset($tbayarangsuran_edit)) $tbayarangsuran_edit = new ctbayarangsuran_edit();

// Page init
$tbayarangsuran_edit->Page_Init();

// Page main
$tbayarangsuran_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tbayarangsuran_edit->Page_Render();
?>
...
var CurrentForm = ftbayarangsuranedit = new ew_Form("ftbayarangsuranedit", "edit");

// Validate form
ftbayarangsuranedit.Validate = function() {
...
ftbayarangsuranedit.Form_CustomValidate = 
...
ftbayarangsuranedit.ValidateRequired = true;
<?php } else { ?>
ftbayarangsuranedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftbayarangsuranedit.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftbayarangsuranedit.Lists["x_active"].Options = <?php echo json_encode($tbayarangsuran->active->Options()) ?>;
...
<?php if (!$tbayarangsuran_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $tbayarangsuran_edit->ShowPageHeader(); ?>
<?php
$tbayarangsuran_edit->ShowMessage();
?>
<form name="ftbayarangsuranedit" id="ftbayarangsuranedit" class="<?php echo $tbayarangsuran_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tbayarangsuran_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tbayarangsuran_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $tbayarangsuran->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="tbayarangsuran" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($tbayarangsuran->id->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $tbayarangsuran->angsuran->EditValue ?></p></span>
</span>
<input type="hidden" data-table="tbayarangsuran" data-field="x_angsuran" name="x_angsuran" id="x_angsuran" value="<?php echo ew_HtmlEncode($tbayarangsuran->angsuran->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tbayarangsuran_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ftbayarangsuranedit.Init();
</script>
<?php
$tbayarangsuran_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tbayarangsuran_edit->Page_Terminate();
?>
