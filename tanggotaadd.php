...
$tanggota_add = NULL; // Initialize page object first

class ctanggota_add extends ctanggota {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tanggotaview.php")
	...
	function LoadDefaultValues() {
		$this->registrasi->CurrentValue = "0000-00-00 00:00:00";
		...
		$this->tanggallahir->CurrentValue = "0000-00-00 00:00:00";
	...
	function AddRow($rsold = NULL) {
		...
		// registrasi
		$this->registrasi->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->registrasi->CurrentValue, 0), ew_CurrentDate(), strval($this->registrasi->CurrentValue) == "");
		...
		// tanggallahir
		$this->tanggallahir->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->tanggallahir->CurrentValue, 0), ew_CurrentDate(), strval($this->tanggallahir->CurrentValue) == "");
		...
		// alamatkantor
		$this->alamatkantor->SetDbValueDef($rsnew, $this->alamatkantor->CurrentValue, "", FALSE);

		// wilayah
		$this->wilayah->SetDbValueDef($rsnew, $this->wilayah->CurrentValue, "", FALSE);
...
// Create page object
if (!isset($tanggota_add)) $tanggota_add = new ctanggota_add();

// Page init
$tanggota_add->Page_Init();

// Page main
$tanggota_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tanggota_add->Page_Render();
?>
...
var CurrentForm = ftanggotaadd = new ew_Form("ftanggotaadd", "add");

// Validate form
ftanggotaadd.Validate = function() {
...
ftanggotaadd.Form_CustomValidate = 
...
ftanggotaadd.ValidateRequired = true;
<?php } else { ?>
ftanggotaadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftanggotaadd.Lists["x_active"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ftanggotaadd.Lists["x_active"].Options = <?php echo json_encode($tanggota->active->Options()) ?>;
...
<?php } ?>
<?php $tanggota_add->ShowPageHeader(); ?>
<?php
$tanggota_add->ShowMessage();
?>
<form name="ftanggotaadd" id="ftanggotaadd" class="<?php echo $tanggota_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tanggota_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tanggota_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="tanggota" data-field="x_id" name="x_id" id="x_id" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($tanggota->id->getPlaceHolder()) ?>" value="<?php echo $tanggota->id->EditValue ?>"<?php echo $tanggota->id->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tanggota_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
